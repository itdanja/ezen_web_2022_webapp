		...
		while (true) { // 무한루프 [ 종료조건 : 승리판단 있을경우 ] // while 1 
			...
			while( true ) { // while 2
				...
				if( 게임판[위치].equals("[ ]")){ 게임판[위치] = "[X]"; break; } // 알을 두면 무한루프 종료
				   ...
				   // 가로로 이기는 수  (012, 345, 678 )
	
				   for( int i= 0 ; i<= 6; i += 3 ){
				    	if( !게임판[i].equals("[ ]") && 게임판[i].equals(게임판[i+1]) && 게임판[i+1].equals(게임판 [i+2])) 
				   	{ System.out.println ( "사용자의 승리 입니다.");   승리판단 = false;
				   ...
				   // 무승부 
					     for( int i=0 ; i <=8 ; i++ ) {
	                            if( !게임판[i].equals("[ ]")) {  
					    	     ...
					    	     if ( i == 9 && 승리판단 == true ) { System.out.println( "무승부 입니다.");}  
